...
Launch all matching investigators in one fan-out phase so they run concurrently. Don't ask one agent to cover multiple MCPs. Route each through the `why investigators, synthesizer` line's descriptor (default `inherit-parent`) with the assigned MCP available. Mechanically read-only modes strip MCP access and disable MCP-backed investigators entirely; investigators still do not write files, which is a posture even when the MCP-capable execution mode is not mechanically read-only.
...
Dispatch one synthesizer through the `why investigators, synthesizer` line's descriptor (default `inherit-parent`). Preserve relevant MCP access because the synthesizer's quality check spot-verifies citations. It does not write files.
