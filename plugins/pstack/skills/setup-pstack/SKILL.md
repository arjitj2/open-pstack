...
Read the model matrix. Every non-alias value must match `<provider>:<model>@<effort>`. Apply the optional Cursor rules above to `cursor:*` values. For baseline descriptors, map it to exactly one matrix family by `(provider, model)`, require its effort to appear in that row's Selectable efforts cell, and collect the effort. `inherit-parent` and `auto` rows carry no family effort.
