...
4. Pick the worker descriptor from the `swarm workers` line in the current harness's pstack model sheet. If the sheet or that line is missing, use `grok:grok-4.7@xhigh`. For `auto` or `inherit-parent`, the workers run on the parent model through its native lane. A configured descriptor that fails its route is a dropout, not a substitution; record it and continue. For a model race, name each arm's descriptor up front.
