...
Take the runners from the `architect runners` line in the current harness's pstack model sheet, in place of the `arena runners` line. If the sheet or that line is missing, use `codex:gpt-5.6-sol@max`, `grok:grok-4.7@xhigh`, `claude:opus@max`. Alias, chain, and dropout entries follow the runner rules in the **arena** skill's Phase A, including the saved fallback state machine.
