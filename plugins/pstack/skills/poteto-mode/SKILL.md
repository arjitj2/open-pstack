...
These skills share one tree across Claude Code and Codex. Read [`references/provider-dispatch.md`](references/provider-dispatch.md) whenever a configured role launches. It defines the provider-qualified model descriptors, sheet grammar, native/external route table, launcher, isolation, receipts, dropout policy, and the saved-chain fallback state machine (`usage-exhausted` only, unless the sheet declares a broader `# fallback` policy). Children never choose routes. When a skill names a Claude tool or built-in skill (`run`, `verify`, `plugin-dev:skill-development`), read [`references/codex-tools.md`](references/codex-tools.md) for the Codex equivalent.
...
For `inherit-parent`, `auto`, or an unconfigured native ad-hoc helper, prefer `poteto-agent`. `/poteto-mode` and `poteto-agent` route through the same wrapper. A provider-qualified role instead follows provider dispatch: Claude's shipped frontier agent definitions cover the native lanes they name by model alias and effort (any other Claude model runs through the external launcher), Codex passes the exact model and effort to `spawn_agent`, and the remaining providers run through the deterministic launcher. Routed workflow skills set the task and access mode. Do not override their choices.

**Defaults for every delegation.** Start independent lanes together, use file pointers rather than inlined dumps, preserve only the tools or MCPs the task needs, and assign every writer a worktree or unique output directory. `/setup-pstack` configures the descriptor per role. Upstream defaults use Grok for the code delegates, exploration, and swarm work; Opus for judgment, prose, explanation, synthesis, and the hardest tasks; and a three-provider frontier panel (Opus, Sol, Grok) for model-diverse judgment. This port's sheet mirrors that shape with portable descriptors (`grok:grok-4.7@xhigh`, `claude:opus@max`, `codex:gpt-5.6-sol@max`), except bug fix, perf issue, and hillclimb stay on `codex:gpt-5.6-sol@max` for cost under the documented upstream exclusion. The panel defaults are enumerated in `arena`, `architect`, and `interrogate`. `inherit-parent` and `auto` use the parent model natively and reduce provider diversity when used in a panel. Each code playbook's configured model comes from its line (`feature, refactoring`, `bug-fix`, `perf-issue`, or `hillclimb`), the hardest changes read `hardest tasks`, and prose and judgment read `judgment and prose`.
