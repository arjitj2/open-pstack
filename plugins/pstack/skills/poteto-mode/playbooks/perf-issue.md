...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation through provider dispatch using your configured perf-issue descriptor (default `codex:gpt-5.6-sol@max`) with `isolated-write` in a dedicated worktree. If the writer attempt ends in an outcome the sheet's `# fallback` policy could advance (`usage-exhausted` only when no policy is saved), follow provider-dispatch's writer rule: if the workload may have started, inspect and preserve its work autonomously and record a clear or unsafe `inspection` verdict. A clear verdict, or proof that the workload never started, allows the parent to ask `next` for the next saved descriptor. Announce the failure and any required inspection first. Only after a clear verdict or proof that no workload started, announce the exact next descriptor approved by `next` and continue without asking for permission. Give a later writer a fresh worktree from the reviewed continuation snapshot, retaining the failed worktree and receipt; an unsafe verdict stops only that lane. Review the diff. Capture a post-fix trace.
