...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation through provider dispatch using your configured perf-issue descriptor (default `codex:gpt-5.6-sol@max`) with `isolated-write` in a dedicated worktree. If the writer attempt ends `usage-exhausted`, follow provider-dispatch's writer rule: stop for inspection unless the receipt proves the model/workload invocation never started, and give any approved later attempt a fresh worktree from the same immutable baseline while retaining the failed attempt's worktree and receipt. Review the diff. Capture a post-fix trace.
