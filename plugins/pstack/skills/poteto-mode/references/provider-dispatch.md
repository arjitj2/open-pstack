...
On macOS, the Claude authentication preflight runs under `/usr/bin/sandbox-exec` with outbound network access denied. This prevents the short-lived status process from consuming an OAuth refresh token before exit. The model invocation retains normal network access; subscription-only authentication and ambient API-credential checks still apply. Sandbox launch failures are terminal, including when an existing parent sandbox rejects nesting. Do not retry the status check without isolation or change the user's credentials. Other operating systems retain their existing preflight.
...
The runner accepts `--api-spend <deny|approved>` and records it in the receipt. `approved` explicitly authorizes a metered API route. `deny` performs a bounded preflight and writes a `billing-policy-blocked` receipt (exit 78, `failurePhase: "preflight"`, `processStarted: false`) when a known API route is present or required subscription-auth evidence cannot be established. Every external attempt from a policy-enabled sheet passes the exact saved value; omission preserves legacy behavior for preexisting providers only on an untouched sheet with no access metadata and no chains. OpenCode has no legacy exemption.
