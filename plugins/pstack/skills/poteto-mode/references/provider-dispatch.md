...
The protocol and configuration behavior were inspected in [OpenCode source at 98325dc](https://github.com/anomalyco/opencode/tree/98325dcdc6a566de6b7ab42cc87af544bed3658d/packages/opencode/src), including `cli/cmd/run.ts`, `config/config.ts`, `config/paths.ts`, `agent/agent.ts`, and `session/prompt.ts`. The local adapter targets OpenCode 1.4.0. Installed-parent validation is a separate release gate.
