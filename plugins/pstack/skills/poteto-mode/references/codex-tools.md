...
OpenCode descriptors (`opencode:<provider>/<model>@default`) always use the external runner. Require explicit API-spend approval. Follow the optional OpenCode contract in `provider-dispatch.md`. Read-only workers have file inspection tools; writers require a Git worktree root and have scoped edits. Neither mode permits shell commands, so the parent runs builds and tests.
