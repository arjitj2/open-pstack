...
Candidate version: **1.6.0**, package Git tree `92947fa71b274333525c34703d69832717828358`. Tracked in [issue #29](https://github.com/arjitj2/open-pstack/issues/29). This candidate is not released. The exact package has passed targeted live worker validation from both Codex and Claude Code.
...
| Fresh Claude Code session | Load the same candidate via supported `--plugin-dir`, invoke `pstack:poteto-mode`, read the dispatch reference, and run both modes through the loaded runner | Both receipts complete, exit 0; read-only took 12.046 seconds and file writing took 21.094 seconds. Independent output, file readback, cleanup, and candidate identity checks passed. |
...
The first Claude attempt failed OAuth authentication. A retry authenticated but ended while a background check was running, which cancelled its worker. Enabling the parent completion-wait tool and requiring it to wait produced the successful run above. Neither failed attempt counts as a pass. No runner or package changes were needed.
...
Local transcripts, receipts, fixture bytes, command records, and the candidate manifest are retained under `~/open-pstack-verification/antigravity-20260925`. The public record omits account and session details. The host-validation blocker is resolved. These results do not merge, tag, release, or roll out the candidate.
