...
Optional Devin, Cursor, and Antigravity CLI workers extend the provider choices available from both parents. Antigravity workers read and edit through file tools, with test execution handled by the parent. Select only the providers you use during setup; adding support does not change saved assignments.
