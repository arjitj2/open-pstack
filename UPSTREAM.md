...
The table records the packaged version and its incorporated Cursor content, with the exclusions below. Version 1.7.0 is an OpenCode worker candidate with installed-parent validation recorded in `docs/opencode-worker-verification.md`; the latest released checkpoint remains 1.5.0. Detecting or reviewing newer Cursor commits does not advance this baseline. `README-UPSTREAM.md` preserves the upstream pstack README verbatim. `CHANGES.md` and `NOTICE.md` describe the adaptations and provenance.
