...
The table records the packaged version and its incorporated Cursor content, with the exclusions below. Version 1.6.0 is an OpenCode worker candidate pending installed-parent validation; the latest tested release remains 1.5.0. Detecting or reviewing newer Cursor commits does not advance this baseline. `README-UPSTREAM.md` preserves the upstream pstack README verbatim. `CHANGES.md` and `NOTICE.md` describe the adaptations and provenance.
