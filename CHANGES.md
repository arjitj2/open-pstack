...
**Shared parser and resolver.** `skills/poteto-mode/scripts/model-policy/` ships the one production parser/resolver and a thin `pstack-model-policy` CLI. `validate --sheet ... --parent ...` checks every candidate row before setup writes it; `resolve` exposes a role for inspection; `next` accepts a frozen sheet plus exact per-lane history, exhausted provider groups, and access mode before every attempt, returning `launch`, `stop`, or `inspect`. Comma-bearing role headers are matched against the documented role list before the right-hand side is split. Unauthorized routes stop instead of being skipped; only a previously exhausted group can be skipped.

**Runner evidence, still one attempt.** Receipts add `failurePhase` and `processStarted` (schema stays 1) plus the recorded `apiSpend` policy. `processStarted: false` means no model/workload invocation launched, although authentication preflight may have invoked the CLI. New terminal statuses: `usage-exhausted` (exit 75) recognizes only the exact supported Codex terminal diagnostics and Grok's exact final free-usage result envelope; Claude, Cursor, Devin, generic 429/403, entitlement/auth/model/task failures, timeouts, cancellations, malformed output, and quota words in generated text remain ordinary failures. `billing-policy-blocked` (exit 78) applies the bounded credential guard. `--api-spend deny` checks known environment routes, including Grok's source-backed `GROK_CODE_XAI_API_KEY`, plus exact sanitized Claude and Codex auth metadata; Claude also excludes user/project/local settings for both checks and invocation. Grok blocks denied API spending because model-level BYOK can override OAuth. Devin and Cursor retain account login checks, isolated configuration, and environment guards including endpoint overrides. `approved` explicitly authorizes and records API spend. Every policy-enabled external launch passes the saved value, while omission preserves untouched legacy behavior. The guard never prints or persists values, does not change ambient auth, and makes no zero-charge claim.

**Parent-owned fallback.** `provider-dispatch.md` now owns the common state machine: freeze the sheet once per run, call `next` before every attempt, retain lane history and global exhausted-provider groups, advance only on proven `usage-exhausted`, bound each attempt to once, and report actual descriptors and provider diversity. Writer lanes stop for inspection unless the receipt proves the model/workload invocation never started; any authorized later attempt gets a fresh worktree from the same immutable baseline. Arena, Architect, Swarm, Interrogate, How, Why, Reflect, and the poteto code playbooks reference that contract instead of absolute no-substitution rules; the controller cannot recover itself when its own account stops.

**Setup discovery.** `setup-pstack` discovers observable CLI/auth/model facts first, uses authoritative provider-reported plan metadata without treating it as remaining capacity, asks only for missing subscription, capacity, and spending facts, reuses confirmed facts and either saved API choice, and recommends task-fit included-capacity assignments plus explicit ordered fallbacks. Every selected provider needs known funding and binding `apiSpend` authorization before save; capacity may remain unknown. Every persisted candidate — primary or fallback, including unchanged legacy assignments — is probed with that saved policy before the existing atomic write. The complete role/chain/funding diff is confirmed before anything is written.
...
Sonnet uses a rolling Claude alias, numeric concrete-revision verification, migration of versioned descriptors, and five native effort agents. Astra, Luna, and Terra use exact Codex models. Native setup probes and smoke candidates observe available capacity, drain and release completed handles before subsequent waves, and run sequentially when capacity is unknown. External lanes remain concurrent; the smoke judge waits for all candidates. No fallback or implicit timeout is added.

The static default-panel check filters First-run active rows. Existing panel defaults are unchanged. Installed behavioral validation remains required before release.
